...
function buildSystem(persona: Persona | undefined): string {
  const lines: string[] = [
    "あなたは九星気学に基づく「今月の運勢ジャーナル」の文章を書くアシスタントです。",
    "以下のルールを厳守してください:",
    "- 方位・運勢・星などの事実は、ユーザーメッセージで与えられる構造化データのみを根拠にする。数値や方位を創作・変更しない。",
    "- 与えられていない占い結果(金運の額、具体的な出来事など)を断定しない。",
    "- 日本語で、200〜280文字程度。今月ひと月の全体的な傾向と、前向きで具体的な「今月の過ごし方」の提案を必ず1つ含める。",
    "- 「今日」ではなく「今月」の視点で書く(1か月を通した流れ・テーマを語る)。",
...
function formatGoodDirections(data: MonthlyStructured): string {
  if (data.goodDirections.length === 0) return "なし";
  return data.goodDirections.map((d) => `${d.level}=${d.label}(${d.starName})`).join("、");
}

function formatBadDirections(data: MonthlyStructured): string {
 ...
 */
function buildUser(data: MonthlyStructured, charName: string): string {
  return [
    "以下の構造化データに基づいて、今月の運勢文を書いてください。",
    "",
    `対象: ${String(data.kigakuYear)}年 気学${String(data.kigakuMonth)}月`,
    `タイプ名: ${data.typeName}`,
    `キャラ名: ${charName}`,
    `本命星: ${data.honmeiStarName}`,
    `月命星: ${data.getsumeiStarName}`,
    `月盤中宮: ${data.monthCenterStarName}`,
